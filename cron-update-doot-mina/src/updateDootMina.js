...
const { TOKEN_TO_CACHE, MINA_CID_CACHE } = require("./utils/constants/info");
  ...
  try {
    ...
    return {
      ...
      data: {
        ...
        settlement_hash: minaResult.settlementTxHash,
    ...
    const elapsed = Math.round((Date.now() - startTime) / 1000);
    cleanup();
    console.error("FATAL ERR! Mina L1 update failed:", error.message || error);
    return {
      status: false,
...
async function updateMinaContractWithPolling(
  ...
  try {
    ...
    console.error("ERR! Mina L1 transaction error:", error.message || error);
    return {
      success: false,
      error: error.message || String(error),
      network: "mina_l1",
