...
export async function updateDootZeko(): Promise<UpdateResult> {
  ...
  try {
    ...
    const elapsed = Math.round((Date.now() - startTime) / 1000);
    cleanup();
    console.error("FATAL ERR! Zeko L2 update failed:", error instanceof Error ? error.message : String(error));
    return {
      status: false,
...
async function updateZekoL2ContractWithPolling(
  ...
  try {
    ...
    if (zekoConfirmed) {
      console.log("Zeko L2 confirmed! Waiting 30 seconds before settlement to avoid nonce conflicts...");
      ...
      try {
        ...
        console.warn(
          `WARN! Zeko L2 settlement failed: ${settlementError instanceof Error ? settlementError.message : String(settlementError)}`
        );
    ...
    console.error("ERR! Zeko L2 transaction error:", error instanceof Error ? error.message : String(error));
    return {
      success: false,
      error: error instanceof Error ? error.message : String(error),
      network: "zeko_l2",
