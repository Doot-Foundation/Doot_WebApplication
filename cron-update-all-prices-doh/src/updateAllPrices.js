...
async function startFetchAndUpdates(tokens) {
  ...
  const historical = await fetchHistoricalData(cid, gateway); // 2 minutes timeout for large IPFS files
  ...
  for (const token of tokens) {
    ...
    try {
      ...
      // Validate token exists in historical data
      if (!historical.data.latest.prices[token]) {
        console.warn(
          `Token ${token} missing from historical latest.prices, using empty array`
        );
      }
