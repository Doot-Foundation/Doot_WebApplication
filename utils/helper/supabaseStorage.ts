...
function ensureConfig(bucket?: string): string {
  if (!SUPABASE_URL) {
...
export async function uploadObject(options: {
  ...
  const { bucket, objectPath, body, contentType = "application/json" } = options;
...
export async function downloadObject(options: {
  ...
  const { bucket, objectPath } = options;
...
async function listObjects(options: { bucket?: string; prefix?: string }) {
...
export async function cleanupPrefixExcept(options: {
  bucket?: string;
  prefix: string;
  keep: string[];
}): Promise<void> {
...
export async function uploadWithHashAndCleanup(options: {
  bucket?: string;
  objectPath: string;
  pointerPath?: string;
  ...
  const { bucket, objectPath, pointerPath, serializedPayload, updatedAt, cleanupPrefix, cid } =
    options;
  const hash = sha256String(serializedPayload);
  ...
  const downloaded = await downloadObject({ bucket, objectPath });
